...
![CREST generates an ensemble, Burette reviews its report and structures, and PRISM can prune a prepared XYZ or SDF ensemble.](/assets/workflow-crest-prism.svg)
...
Open **Conformers → Settings → Check** to see which executables Burette discovered. CREST and PRISM are not bundled with the app.
...
CREST runs in a numbered `crest_run_N` folder beside a file-backed source when possible. Burette writes a report and combined log, collects recognized ensemble and fallback artifacts, opens the primary reviewable result, and records the job state.

If CREST fails during initial geometry optimization, Burette can retry after an xTB pre-optimization. It can also retry without implicit solvent after that pre-optimization, or fall back to GFN-FF for a qualifying initial-geometry failure. A recovered structure is marked as recovery output in the report; review it before treating it as a conformer ensemble.
...
Burette recognizes multi-frame XYZ and multi-record SDF/SD ensembles. The conformer summary reports the detected source, conformer count, atom count, available energies, relative energy range, cluster markers, and pipeline recommendations.
...
For CREST, start with a small neutral organic molecule and a fast sampling preset. Confirm that Jobs completes, `crest_run_N` contains the report and log, and Burette opens a multi-conformer output or clearly labels a recovered fallback.
